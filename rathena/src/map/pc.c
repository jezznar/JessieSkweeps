...
char pc_payzeny(struct map_session_data *sd,int zeny, enum e_log_pick_type type, struct map_session_data *tsd)
{
	...
	/** Tax system
	 * gets a certain % of the zeny if LOG_TYPE_NPC
	 * or LOG_TYPE_VENDING
	 * and puts it in SQL: Map_Tax: <mapindex> <owner> <amount of tax>
	 */
//#define taxsystem
#ifdef taxsystem
	//read tax% and taxz
	float taxp = Sql_Query(mmysql_handle, "SELECT tax from map_tax where mapindex='%s';", sd->mapindex);
	int taxz = Sql_Query(mmysql_handle, "SELECT tax_z from map_tax where mapindex='%s';", sd->mapindex);
	taxz += (int)(taxp * zeny);
	switch (type){
	case LOG_TYPE_NPC:
	case LOG_TYPE_VENDING:
		Sql_Query(mmysql_handle, "UPDATE map_tax SET tax_z='%d' WHERE mapindex='%s';", taxz, sd->mapindex);
	}
#endif
	return 0;
}
